...
class IrLowerer(
        ...
        for (stdProgram in StdlibAst.programs) {
            for (singleton in stdProgram.singletons) {
                ...
                for (function in singleton.functions.filterNot { it.internal }) {
                    val symbol = globals.resolveMember(typeQualifiedName, function.name, functionKind(function))
    ...
    private fun lowerMemberFunctionCall(
        ...
        val resolution = resolveMemberCall(call, symbols, context)
    ...
    private fun resolveMemberCall(
        ...
        val resolverContext = functionResolver.contextForSelection(context?.currentSelection())
        ...
        if (staticType != null) {
            val symbol = functionResolver.resolveMember(staticType.qualifiedName, call.name, resolverContext)
            if (!symbol.isStaticMember) {
                error("Member function '${call.name}' requires an instance")
            }
            return MemberResolution(symbol, null)
