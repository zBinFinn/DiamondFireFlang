...
class TypeChecker(
    ...
    private fun checkMemberFunctionCall(
        ...
        val symbol = if (staticType != null) {
            resolveMember(staticType.qualifiedName, call.name, program, functionName, activeSelection, diags)?.also {
            ...
            if (receiverType !is Type.Dict && receiverType !is Type.Singleton && receiverType !is Type.ListType && receiverType !is Type.DictionaryType) {
                ...
                resolveMember(receiverTypeName, call.name, program, functionName, activeSelection, diags)?.also {
                    if (it.isStaticMember) {
                        diags += Diagnostic(
    ...
    private val gameValueReturnTypes: Map<String, Type> by lazy {
        val text = Thread.currentThread().contextClassLoader
            ?.getResource("action_dump.json")
            ?.readText()
            ?: File("compiler/src/main/resources/action_dump.json")
                .takeIf { it.exists() }
                ?.readText()
            ?: File("src/main/resources/action_dump.json")
                .takeIf { it.exists() }
                ?.readText()
    ...
    private fun resolveMember(
        ...
        return try {
            functionResolver.resolveMember(
                typeQualifiedName,
                name,
                functionResolver.contextForSelection(activeSelection),
            )
        } catch (t: Throwable) {
            diags += Diagnostic(
