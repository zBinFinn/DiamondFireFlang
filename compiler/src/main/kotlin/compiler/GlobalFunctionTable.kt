...
class GlobalFunctionTable {
    ...
    private val byMember = mutableMapOf<String, MutableMap<String, MutableList<FunctionSymbol>>>()
    ...
    fun resolveMember(typeQualifiedName: String, name: String, kind: FunctionKind = FunctionKind.Plain): FunctionSymbol? =
        byMember[typeQualifiedName]?.get(name)?.singleOrNull { it.kind == kind }

    fun resolveMemberCandidates(typeQualifiedName: String, name: String): List<FunctionSymbol>? =
        byMember[typeQualifiedName]?.get(name)

    fun membersFor(typeQualifiedName: String): Map<String, List<FunctionSymbol>> =
        byMember[typeQualifiedName] ?: emptyMap()

    fun allFunctions(): Set<FunctionSymbol> =
        byQualified.values.flatten().plus(byMember.values.flatMap { it.values.flatten() }).toSet()
    ...
    private fun registerImpl(modulePath: String, impl: Ast.ImplDecl, program: Ast.Program) {
        ...
        for (fn in impl.functions) {
            ...
            val existing = memberMap.getOrPut(fn.name) { mutableListOf() }
            if (existing.any { it.kind == FunctionKind.Plain }) {
    ...
    private fun registerSingleton(modulePath: String, singleton: Ast.SingletonDecl, program: Ast.Program) {
        ...
        for (fn in singleton.functions) {
            ...
            if (existing.any { it.kind == kind }) {
                error("Duplicate $kind member function '${fn.name}' for '$typeQualifiedName'")
            }
