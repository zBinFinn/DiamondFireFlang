...
class ParseDiagnosticException(
    val diagnostic: Diagnostic,
) : RuntimeException(diagnostic.message)

class Parser(
    private val tokens: List<Token>,
    private val moduleHint: String = "<unknown>",
) {
    private var index = 0
    ...
    private fun fail(message: String): Nothing {
        val token = peek()
        throw ParseDiagnosticException(
            Diagnostic(
                message = message,
                module = moduleHint,
                range = token.range,
            ...
            when {
                ...
                peek().type == TokenType.AT -> {
                    ...
                    when (peek().type) {
                        ...
                        else -> fail("Expected function or singleton after annotation at ${peek().position}")
                    }
                }
                ...
                else -> fail("Unexpected token '${peek()}'")
            }
        }
    ...
    private fun parseField(): Ast.Field {
        val mutable = when {
            ...
            else -> fail("Expected 'val' or 'var' before field name at ${peek().position}")
        }
    ...
    private fun parseFunctionParameter(implicitThisType: Ast.Type? = null): Ast.Parameter {
        ...
        if (identifier == "this") {
            if (implicitThisType == null) {
                fail("'this' parameters are only allowed inside impl blocks")
            }
            if (peek().type == TokenType.COLON) {
                fail("'this' parameter type is inferred from the impl block")
            }
    ...
    private fun parseStatement(): Ast.Statement {
        when (peek().type) {
            ...
            TokenType.WITH -> {
                ...
                val functionCall =
                    tryParseFunctionCall(expectSemi = false) ?: fail("Expected selector function call after 'with'")
            ...
            TokenType.IDENT -> {
                ...
                fail("Expected function call statement")
            }

            else -> fail("Unexpected token ${peek()} for statement")
        }
    }
    ...
    private fun parsePrimaryExpression(): Ast.Expr {
        return when (peek().type) {
            ...
            TokenType.IDENT -> {
                ...
                if (identifier == "gval" && match(TokenType.LPAREN)) {
                    ...
                    if (nameExpr !is Ast.StringExpr) {
                        fail("gval name must be a string literal")
                    }
            ...
            else -> fail("Unexpected Token ${peek()} for expression")
        }
    }
